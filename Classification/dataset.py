...
def cifar10_dataloaders(
    ...
    parsed_classes_to_replace = parse_classes_to_replace(classes_to_replace)
    if parsed_classes_to_replace and indexes_to_replace is not None:
        raise ValueError(
            "Only one of `classes_to_replace` and `indexes_to_replace` can be specified"
        )
    if class_to_replace is not None and indexes_to_replace is not None:
        raise ValueError(
            "Only one of `class_to_replace` and `indexes_to_replace` can be specified"
        )
    if parsed_classes_to_replace:
        replace_classes(
            train_set,
            parsed_classes_to_replace,
            num_indexes_to_replace=num_indexes_to_replace,
            seed=seed - 1,
            only_mark=only_mark,
        )
        if num_indexes_to_replace is None or num_indexes_to_replace == 4500:
            retain_mask = ~np.isin(test_set.targets, parsed_classes_to_replace)
            test_set.data = test_set.data[retain_mask]
            test_set.targets = test_set.targets[retain_mask]
    elif class_to_replace is not None:
        replace_class(
            train_set,
            class_to_replace,
            num_indexes_to_replace=num_indexes_to_replace,
...
def parse_classes_to_replace(classes_to_replace):
    if classes_to_replace is None:
        return []
    if isinstance(classes_to_replace, str):
        return [
            int(item.strip())
            for item in classes_to_replace.split(",")
            if item.strip()
        ]
    return [int(item) for item in classes_to_replace]
...
def replace_classes(
    ...
    classes_to_replace = parse_classes_to_replace(classes_to_replace)
    if not classes_to_replace:
        return

    if -1 in classes_to_replace:
        try:
        ...
        try:
            indexes = np.flatnonzero(np.isin(np.array(dataset.targets), classes_to_replace))
        except:
            try:
                indexes = np.flatnonzero(np.isin(np.array(dataset.labels), classes_to_replace))
            except:
                indexes = np.flatnonzero(np.isin(np.array(dataset._labels), classes_to_replace))
