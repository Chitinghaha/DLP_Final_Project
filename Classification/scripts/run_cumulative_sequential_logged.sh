...
MASK_SCOPE="${MASK_SCOPE:-cumulative}"
...
RUN_ID="${RUN_ID:-${MASK_SCOPE}_seed${SEED}_k${MAX_K}_$(date +%Y%m%d_%H%M%S)}"
LOG_DIR="${LOG_DIR:-results/logs/sequential_cumulative/${RUN_ID}}"
ORIGINAL_MODEL="${ORIGINAL_MODEL:-results/original/cifar10_resnet18_seed${SEED}/0model_SA_best.pth.tar}"

if [[ "${MASK_SCOPE}" != "cumulative" && "${MASK_SCOPE}" != "newclass" ]]; then
  echo "MASK_SCOPE must be 'cumulative' or 'newclass', got '${MASK_SCOPE}'" >&2
  exit 1
fi

mkdir -p "${LOG_DIR}" \
  "results/masks/sequential_cumulative/${MASK_SCOPE}/seed${SEED}" \
  "results/unlearn/sequential_cumulative/${MASK_SCOPE}/seed${SEED}" \
  "results/eval/sequential_cumulative/${MASK_SCOPE}"
...
log_summary "Settings: SEED=${SEED} GPU=${GPU} ARCH=${ARCH} DATASET=${DATASET} MAX_K=${MAX_K} MASK_SCOPE=${MASK_SCOPE} UNLEARN_EPOCHS=${UNLEARN_EPOCHS}"
...
for step in $(seq 1 "${MAX_K}"); do
  ...
  slug="${forgotten//,/_}"
  mask_dir="results/masks/sequential_cumulative/${MASK_SCOPE}/seed${SEED}/step${step}_forget_${slug}"
  save_dir="results/unlearn/sequential_cumulative/${MASK_SCOPE}/seed${SEED}/step${step}_forgot_${slug}"
  eval_csv="results/eval/sequential_cumulative/${MASK_SCOPE}/seed${SEED}_step${step}_forgot_${slug}.csv"
...
log_summary "DONE all cumulative sequential stages"
