...
RESULT_NAMESPACE="${RESULT_NAMESPACE:-sequential}"
...
remaining.extend(stages[stage_index + 1:])
...
for name in remaining:
    if averages[name] is None:
        ...
        fallback += averages[name]
print("unknown" if unknown else round(fallback, 2))
PY
}
...
log_summary "Settings: SEED=${SEED} GPU=${GPU} ARCH=${ARCH} DATASET=${DATASET} MAX_K=${MAX_K} UNLEARN_EPOCHS=${UNLEARN_EPOCHS} RESULT_NAMESPACE=${RESULT_NAMESPACE}"
...
if [[ -n "${FORGET_ORDER}" ]]; then
  if [[ "${#FORGET_ORDER_ARRAY[@]}" -lt "${MAX_K}" ]]; then
    echo "FORGET_ORDER must contain at least MAX_K=${MAX_K} entries, got ${#FORGET_ORDER_ARRAY[@]}" >&2
    exit 1
  fi
fi
...
log_summary "DONE all sequential stages"
